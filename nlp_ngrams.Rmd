...
terms1<-freq[head(ord,freqTerms)]

totalTerm1Occ<-sum(terms1)

terms1_df<-as.data.frame(c("terms","prob"))

# names(terms1)
terms1_df<-cbind(names(terms1),terms1/totalTerm1Occ)

# dim(terms1_df)
colnames(terms1_df)<-c("terms1","prob")
...
totalFreq2g<-length(freq2g[freq2g>10])
#create sort order (descending)
...
terms2_df<-as.data.frame(c("terms2g","prob"))

terms2_df<-cbind(names(terms2g),terms2g/totalTerm2Occ)

colnames(terms2_df)<-c("terms2g","prob")
...
terms3_df<-as.data.frame(c("terms3g","prob"))

terms3_df<-cbind(names(terms3g),terms3g/totalTerm3Occ)

colnames(terms3_df)<-c("terms3g","prob")
...
term4outputFile<-"data/output/term4.txt"
con<-file(term4outputFile,"w")
writeLines(names(terms4g),con)
# write.csv(names(terms4g),con)
close(con)
```
...
options(mc.cores=1)
start_time<-Sys.time()
dtm.5g <- DocumentTermMatrix(docs, control=list(tokenize=FivegramTokenizer))
end_time<-Sys.time()
